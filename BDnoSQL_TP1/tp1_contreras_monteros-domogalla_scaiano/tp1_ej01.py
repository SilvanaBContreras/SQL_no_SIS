...
def procesar_fila(database, fila):
    if fila["id_deportista"] in ["10", "20", "30"]:
        deportista = (fila["nombre_deportista"], fila["id_deportista"])
        if deportista not in database:
...
def generar_reporte(database):
    archivo = open(nombre_archivo_resultado_ejercicio, 'w')
    grabar_linea(archivo, "Deportistas con id 10, 20 y 30:")
    database.sort()
    for deportista in database:
        linea = deportista[1] + "," + deportista[0]
        grabar_linea(archivo, linea)
    archivo.close()
