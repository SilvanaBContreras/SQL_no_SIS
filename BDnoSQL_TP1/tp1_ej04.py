...
def procesar_fila(database, fila):
    if fila["nombre_deportista"] == "Christian COLEMAN" and fila["nombre_ciudad_torneo"] == "Buenos Aires":
        ...
        if especialidad not in database:
            database.append(especialidad)
...
def generar_reporte(database):
    ...
    grabar_linea(
        archivo,
        "Especialidades practicadas por Christian COLEMAN en Buenos Aires:"
    )
    database.sort()
    for especialidad in database:
        linea = especialidad[0] + "," + especialidad[1]
        grabar_linea(archivo, linea)
    archivo.close()
