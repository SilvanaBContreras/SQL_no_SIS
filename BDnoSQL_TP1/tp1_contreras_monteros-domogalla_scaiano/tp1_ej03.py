...
def generar_reporte(database):
    archivo = open(nombre_archivo_resultado_ejercicio, 'w')
    grabar_linea(archivo, "Cantidad de especialidades asociadas a cada tipo:")
    database.sort()
    tipos = []
    # tipos diferentes
    ...
    # cuántas especialidades tiene cada tipo
    for tipo in tipos:
        cantidad = 0
        for especialidad in database:
            if especialidad[0] == tipo:
                cantidad = cantidad + 1
        linea = tipo + "," + str(cantidad)
        grabar_linea(archivo, linea)
    archivo.close()
